...
class PointCloudDataset(Dataset):
    ...
    def __getitem__(self, index):
        np.random.seed(0)
        sampler = np.random.choice(self.total_points, size = int(1*self.total_points), replace = False)
...
class Transform(nn.Module):
   ...
   def forward(self, input):
        ...
        #[2,64,32]
        xb = F.relu(self.conv2(matrixnx64))
        #[2,128,32]
        xb = self.conv3(xb)
        #[2,1024,32]
...
def show_trained_moel(model_save_path, figure_save_path = None):
    ...
    leaf_xyz_pcd.points = o3d.utility.Vector3dVector(points.detach().numpy().squeeze().T)
    # colors = np.random.rand(np.int64(predict_labels.max())+1, 3)
    colors= np.array([[0,0,0],[255,0,0]])/255
    leaf_xyz_pcd.colors = o3d.utility.Vector3dVector(colors[predict_labels.squeeze(), :])


    o3d.visualization.draw_geometries([leaf_xyz_pcd])
...
if __name__ == '__main__':
    ...
    # train(pointnet, train_loader, valid_loader,  epochs=20000, save=True)
    
    show_trained_moel(model_save_path = f"./trainingtest/best_model.pth")
