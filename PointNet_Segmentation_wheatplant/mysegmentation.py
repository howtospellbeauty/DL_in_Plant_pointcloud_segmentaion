...
def pointnetsegmentationloss(output, labels, m3x3, m64x64, alpha = 0):
    ...
    output = output.transpose(1,2).reshape(-1, 2)
...
#epoch，轮。每次使用过全部训练数据完成一次前后向训练称为完成了一轮
    for epoch in range(epochs): 
        ...
        if val_loader:
            with torch.no_grad():
                for points, labels in train_loader:
                    ...
                    predicted_label = torch.argmax(output.transpose(1, 2).reshape(-1, 2), dim = -1)
                    labels = labels.reshape(-1)
        ...
        if epoch % 10 == 0:
            if save:
                torch.save(pointnet.state_dict(), "./trainingresult/save_"+str(epoch)+".pth")


def show_trained_moel():
    pointnet = PointNet()
    pointnet.to(device)
    pointnet.load_state_dict(torch.load('./trainingresult/save_1390.pth'))
...
if __name__ == '__main__':
    ...
    pointnet = PointNet()
    pointnet.to(device);
    optimizer = torch.optim.Adam(pointnet.parameters(), lr=0.0000001)
